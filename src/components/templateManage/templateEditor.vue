<template >
    <!-- 模板编辑界面 -->
    <div class="pb-5" style="overflow-y: auto;overflow-x: hidden;height: 950px" >
        <!-- 模版编辑栏 -->
        <b-row >
            <b-col>
                <b-button-group style="display: flex; position: fixed;z-index: 1071;">
                    ...
                    <b-dropdown text="保存" split  ref="dropdown" @click="save">
                        <b-dropdown-form>
                            <b-form-group>
                                <b-form-checkbox switch @click="autoSave">自动保存</b-form-checkbox>
                            </b-form-group>
                        </b-dropdown-form>
                    </b-dropdown>
                    <b-dropdown id="dropdown-form" text="设置" ref="dropdown" >
                        <b-dropdown-form>
                            <b-form-group label="备注" label-for="dropdown-form-name" @submit.stop.prevent description="默认为空白">
                                <b-form-input
                ...
                <b-row cols="12">
                    ...
                    <b-col cols="8" align-self="center">
                        ...
                        <b-input-group v-if="item.type=='title'" >
                            <b-form-input placeholder="请输入内容" class="textarea" size="lg" :plaintext="item.status" v-model="item.content"></b-form-input>
                        </b-input-group>

                        <!-- 二级标题/三级标题 -->
                        <b-input-group v-if="item.type=='subhead'||item.type=='thirdhead'">
                            <b-form-input placeholder="请输入内容" :plaintext="item.status" v-model="item.content" style="font-weight: bold;"></b-form-input>
                            <b-input-group-append is-text>
                                <b-form-checkbox switch class="mr-n2" v-model="item.status" ></b-form-checkbox>
                            </b-input-group-append>
                        </b-input-group>

                        <!-- 段落 -->
                        <b-input-group v-if="item.type=='paragraph'">
                            <b-form-textarea placeholder="请输入内容" :plaintext="item.status" v-model="item.content" rows="2" max-rows="10"></b-form-textarea>
                            <b-input-group-append is-text>
                                <b-form-checkbox switch class="mr-n2" v-model="item.status" ></b-form-checkbox>
                            </b-input-group-append>
                        </b-input-group>
                        ...
                        <b-table-simple class="mt-3" v-if="item.type=='table'&&item.content.data.length!=0" hover small caption-top responsive bordered>
                            <caption>
                                <b-input-group prepend="表格注释">
                                     ...
                                     <b-input-group-append is-text>
                                        <b-form-checkbox switch class="mr-n2" v-model="item.status.tablenote" ></b-form-checkbox>
                                    </b-input-group-append>
                                </b-input-group>
                            </caption>
                            ...
                            <b-thead>
                                <b-tr>
                                    ...
                                    <b-input-group-append is-text align-self="center">
                                        <b-form-checkbox switch class="m-1 mt-2" v-model="item.status.header" ></b-form-checkbox>
                                    </b-input-group-append>
                                </b-tr>
                            </b-thead>
                            ...
                            <b-tbody>
                                <b-tr v-for="(data,i) in item.content.data" :key="i">
                                    <b-td v-for="(it,n) in data" :key="n" class="text-center" >
                                        <b-form-textarea v-model="data[n]" style="border-width: 0;" rows="2" max-rows="9" no-resize></b-form-textarea>
                                    </b-td>
...
export default {
    ...
    beforeCreate:function(){
        if(this.$route.params.templateId){
            //获取相应id的模板内容
            getTemplateById(this.$route.params.templateId).then(res=>{
    ...
    beforeRouteLeave:function(to, from , next){
        next(false) // 问问这里为什么作用是什么
        if(this.count>1){
            this.$confirm('您还未保存页面内容，确定退出吗?', '提示', {
        ...
        save(){
            ...
            if(this.$route.params.templateId){
                ...
                newTemplate({
                    ...
                    this.date=nowTime();
                    autoId().then(res=>{
                        this.$route.params.templateId = res.data;
                        console.log(res.data);
